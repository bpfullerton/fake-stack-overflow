...
/**
 * Interface extending the request body when authenticating a user, which contains:
...
/**
 * Type representing the possible responses for an Auth operation.
 */
...
export interface CreateThreadRequest extends Request {
  body: {
    qid: string;
    username: string;
    threadId:string;
...
export interface GetThreadsRequest extends Request{
  body:{
    qid:string;
  }
}

export interface DeleteThreadRequest extends Request {
  body: {
    qid: string;
    thread_id:string;
  };
...
export type QuestionThreadResponse = string[] | { error: string };
...
export type UserResponse = {token:string} | { error: string };