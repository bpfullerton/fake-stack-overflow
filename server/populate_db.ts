...
const populate = async () => {
  try {
    ...
    const a1 = await answerCreate(A1_TXT, 'hamkalo', new Date('2023-11-20T03:24:42'), [c1], ['thread1', 'thread2']);
    const a2 = await answerCreate(A2_TXT, 'azad', new Date('2023-11-23T08:24:00'), [c2], ['thread1', 'thread2']);
    const a3 = await answerCreate(A3_TXT, 'abaya', new Date('2023-11-18T09:24:00'), [c3], ['thread1', 'thread2']);
    const a4 = await answerCreate(A4_TXT, 'alia', new Date('2023-11-12T03:30:00'), [c4], ['thread1', 'thread2']);
    const a5 = await answerCreate(A5_TXT, 'sana', new Date('2023-11-01T15:24:19'), [c5], ['thread1', 'thread2']);
    const a6 = await answerCreate(A6_TXT, 'abhi3241', new Date('2023-02-19T18:20:59'), [c6], ['thread1', 'thread2']);
    const a7 = await answerCreate(A7_TXT, 'mackson3332', new Date('2023-02-22T17:19:00'), [c7], ['thread1', 'thread2']);
    const a8 = await answerCreate(A8_TXT, 'ihba001', new Date('2023-03-22T21:17:53'), [c8], ['thread1', 'thread2']);
    ...
    const STREAM_API_KEY = 'eb4nygr27qx2';
    const STREAM_API_SECRET = 'kxhm353grtx9se398937vtg3hqx77tr8m3mndzkdqunurrppyhxewxanu7qfsuwd';

    /**Make a minifunction that takes in name + token and logs into stream chat */
    const client= new StreamChat(STREAM_API_KEY, STREAM_API_SECRET);
    let users: { id: string, name: string }[] = [];
    async function newUser(name: string, token: string) {
      name=name.replace(" ", "_");
      users.push({ id:name, name:name });
      await userCreate(name, token);
    }

    
    await newUser('sana', 'abcde');
    await newUser('saltyPeter', 'bcdef');
    await newUser('hamkalo', 'cdefg');
    await newUser('ihba001', 'defgh');
    await newUser('monkeyABC', 'efghi');
    await newUser('azad', 'fghij');
    await newUser('alia', 'ghijk');
    await newUser('abhi3241', 'hijkl');
    await newUser('Joji John', 'ijklm');
    await newUser('abaya', 'jklmn');
    await newUser('mackson3332', 'klmno');
    await newUser('elephantCDE', 'lmnop');
    console.log(users);
