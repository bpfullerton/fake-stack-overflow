...
/**
 *  Add a Chat ID to a question.
 *
 * @param {string} qid - The ID of the question to add a chat ID to
 * @param {string} threadId - The chat ID to add
 */
export const addChatToQA = async (
  qid: string,
  threadId: string,
  type: 'question' | 'answer',
): Promise<QuestionResponse | AnswerResponse> => {
  try {
    ...
    if (result === null) {
      throw new Error('Error when adding chat to question');
    }
    return result;
  } catch (error) {
    return { error: 'Error when adding chat to question' };
  }
};

/**
 * Get a Chat ID from a question.
 *
 * @param {string} qid - The ID of the question to get the chat ID from
 */
export const getChatFromQA = async (
  qid: string,
  type: 'question' | 'answer',
): Promise<QuestionThreadResponse> => {
  try {
    let result: QuestionThreadResponse | null;

    if (type === 'question') {
      result = await QuestionModel.findOne({ _id: qid });
    } else {
      result = await AnswerModel.findOne({ _id: qid });
    }

    if (result === null) {
      throw new Error('Error when getting threads from question');
    }
    return result;
  } catch (error) {
    return { error: 'Error when getting threads from question' };
  }
};

/**
 * Delete a Chat ID from a question.
 *
 * @param {string} qid - The ID of the question to delete the chat ID from
 * @param {string} chatId - The chat ID to delete
 */
export const deleteChatFromQA = async (
  qid: string,
  threadId: string,
  type: 'question' | 'answer',
  ...
  try {
    ...
    if (result === null) {
      throw new Error('Error when deleting chat from question');
    }
    return result;
  } catch (error) {
    return { error: 'Error when deleting chat from question' };
  }
};

/**
 * Create a new User from a username and token.
 *
 * @param {string} username - The username of the user
 * @param {string} token - The token of the user
 */
export const createUser = async (username: string): Promise<string | null> => {
  try {
    await streamClient.upsertUser({ id: username, name: username });
    const token = await streamClient.createToken(username);

    await UserModel.create({ username, token });
