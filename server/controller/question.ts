...
const questionController = (socket: FakeSOSocket) => {
  ...
  /**
   * Creates a new thread on a question. The request must contain the question ID (qid) and the username.
   * If the request is invalid or an error occurs, the appropriate HTTP response status and message are returned.
   *
  ...
  const createThread = async (req: CreateThreadRequest, res: Response): Promise<void> => {
    const { qid, threadId } = req.body;
    try {
      const status = await addChatToQA(qid, threadId, 'question');
      if (status && 'error' in status) {
        throw new Error(status.error as string);
      }
  ...
  /**
   * Gets a list of threads for a question. The request must contain the question ID (qid).
   * If the request is invalid or an error occurs, the appropriate HTTP response status and message are returned.
   *
  ...
  const getThreads = async (req: GetThreadsRequest, res: Response): Promise<void> => {
    const { qid } = req.body;
    try {
      const threads = await getChatFromQA(qid, 'question');
      if ('error' in threads) {
